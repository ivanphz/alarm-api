...
test("done 事件: 冷却窗内 false, 5小时后恢复 true + Gate-AIQ 提醒", async () => {
  const ld = loaders([{ at: "2026-07-15 09:00", id: "u1", type: "done" }]);
  const mid = await call("date=2026-07-15&now=11:00", ld);
  assert.equal(mid.fields.ai_available.value, false);
  const after = await call("date=2026-07-15&now=14:01", ld);
  assert.equal(after.fields.ai_available.value, true);
  assert.equal(after.fields.ai_available.from, "2026-07-15 14:00");   // 09:00+300min
  // 提醒闹钟: 恢复时刻在窗口内（now=11:00 → 窗口含 14:00）
  const dyn = mid.alarms.dynamic.filter((a) => a.label.startsWith("Gate-AIQ"));
  assert.deepEqual(dyn, [{ label: "Gate-AIQ-ai_claude-1400", at: "2026-07-15 14:00",
...
test("重叠 done 合并 + 周一 08:00 周重置截断冷却", async () => {
  ...
  assert.equal(merged.fields.ai_available.value, false);            // 14:00 已被 b 延到 16:00
  const monday = await call("date=2026-07-20&now=08:01", ld);
  assert.equal(monday.fields.ai_available.value, true);
  assert.equal(monday.fields.ai_available.from, "2026-07-20 08:00"); // 周重置而非 11:30
});
...
test("纠偏事实: reset 立即恢复；set_next 手动指定", async () => {
  ...
  assert.equal(b.fields.ai_available.value, true);                   // reset 截断
  const c = await call("date=2026-07-15&now=17:00", ld);
  assert.equal(c.fields.ai_available.value, false);                  // set_next 阻塞中
  const d = await call("date=2026-07-15&now=18:31", ld);
  assert.equal(d.fields.ai_available.value, true);
});
...
test("事实源降级 → ai_available 无主张(null)，其余字段不受影响", async () => {
  ...
  assert.equal(b.fields.ai_available.value, null);
