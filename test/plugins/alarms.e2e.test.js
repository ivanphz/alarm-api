...
test("R2.1 暑假工作日: SchoolBreak 起床组(含副铃) + 午休/下班铃; Workday 组 off", async () => {
  ...
  assert.ok(on(b, "GateFix-SchoolBreak-WakeUp-Vib"));
  assert.ok(on(b, "GateFix-SchoolBreak-WakeUp-Ring"));   // BUNDLED 联动
  assert.ok(on(b, "GateFix-Workday-NapEnd-Vib"));
  assert.ok(on(b, "GateFix-Workday-OffWork-Vib"));
  assert.ok(!on(b, "GateFix-Workday-WakeUp-Vib"));       // 暑假换组
});
...
test("R2.2/R2.3 非假期节后首日: Workday 组 + FirstWorkday 兜底并行", async () => {
  ...
  assert.ok(on(b, "GateFix-Workday-WakeUp-Vib"));
  assert.ok(on(b, "GateFix-Workday-WakeUp-Ring"));
  assert.ok(on(b, "GateFix-FirstWorkday-WakeUp-Ring"));   // R2.3
});

test("R3 周六舞蹈课(合并后课表 id=Sat-Dance): 暑假==锚走固定; 寒假未配时段不上课", async () => {
  const b = await call("date=2026-07-17&now=22:00");         // 周五晚, 窗口含周六 07:45
  assert.ok(on(b, "GateFix-Class-Sat-Dance"));            // R3.1a summer 07:45==锚
  ...
  assert.ok(!on(w, "GateFix-Class-Sat-Dance"));           // R3.3 该时段不上课
  assert.ok(!w.alarms.dynamic.some((a) => a.label.startsWith("GateDyn-Class")));
});
...
test("R4 全天请假: 早间组/午休/下班全灭, 但夜里窗口内明天照常", async () => {
  ...
  assert.ok(!on(noon, "GateFix-SchoolBreak-WakeUp-Vib"));
  assert.ok(!on(noon, "GateFix-Workday-NapEnd-Vib"));
  assert.ok(!on(noon, "GateFix-Workday-OffWork-Vib"));
  const night = await call("date=2026-07-15&now=20:00", ld);  // 窗口滑进周四 → 周四正常 on
  assert.ok(on(night, "GateFix-SchoolBreak-WakeUp-Vib"));
});

test("R5.1 晨间会议: 关早间组 + 建 GateDyn-Event-0730", async () => {
  const ld = loaders({ async loadCalendars(env, opts, span) {
    const s = await import("../../src/edge/sources.js");
    return s.resolveInstances(
  ...
  assert.ok(!on(b, "GateFix-SchoolBreak-WakeUp-Vib"));     // 晨间碰撞不注入
  assert.deepEqual(b.alarms.dynamic.filter((a) => a.label.startsWith("GateDyn-Event")),
    [{ label: "GateDyn-Event-0730", at: "2026-07-15 07:30", reason: "[会议]" }]);
  assert.ok(on(b, "GateFix-Workday-NapEnd-Vib"));          // 午休/下班不受晨间碰撞影响
});

test("R1 上帝模式: 当日完全接管（闹钟集合与 quiet 全按 JSON）", async () => {
  const god = JSON.stringify({
    fixedAlarms: [{ label: "GateFix-Workday-WakeUp-Vib", action: "ON" }],
    dynamicAlarms: [{ label: "GateDyn-Event-0611", time: "06:11", reason: "god" }],
  ...
  assert.ok(on(b, "GateFix-Workday-WakeUp-Vib"));          // 暑假日却按 god 开 Workday 组
...
test("外部闹钟: 时区换算跨天 + uid 编入标签 + 窗口裁剪", async () => {
  ...
  assert.deepEqual(b.alarms.dynamic.filter((a) => a.label.startsWith("GateDyn-ES")),
...
test("R1 大字报: 真非法 JSON → 回落常规 + trace 定位错误", async () => {
  const ld = loaders({ async loadCalendars(env, opts, span) {
    const s = await import("../../src/edge/sources.js");
    return s.resolveInstances(
      [{ title: "[上帝模式]", startDate: "2026-07-15", endDate: "2026-07-16",
         description: '{ "fixedAlarms": [ { "label": broken } ] }' }],
      span.start, span.end);
  } });
  const b = await call("date=2026-07-15&now=05:00", ld);
  assert.ok(b.trace.some((t) => t.includes("god_json_invalid") && t.includes("2026-07-15")));
  assert.ok(on(b, "GateFix-SchoolBreak-WakeUp-Vib"));         // 常规 R2.1 照常
});

test("R1 真实 ICS 转义链路: RFC5545 的 \\, \\n 反转义后 god JSON 复活（2026-07-22 实案）", async () => {
  // 复刻用户日历导出的原始转义形态（parseICS 前的 DESCRIPTION 内容）
  const ics = [
    "BEGIN:VCALENDAR",
    "BEGIN:VEVENT",
  ...
  const ld = loaders({ async loadCalendars(env, opts, span) {
    const s = await import("../../src/edge/sources.js");
    const { parseICS } = await import("../../src/ics-parser.js");
    return s.resolveInstances(parseICS(ics), span.start, span.end);
  } });
  const b = await call("date=2026-07-15&now=05:00&debug=1", ld);
