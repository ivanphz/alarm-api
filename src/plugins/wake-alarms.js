...
//   R5.1 晨间 work_event 有具体时间 → Gate-Dynamic-Event-<HHMM>（任何日子都建，与 v1 同）
  ...
  produce(ctx, range) {
    ...
    for (let d = range.start; d <= range.end; d = addDays(d, 1)) {
      ...
      if (p.workday) {
        if (p.morning === "work") {
          if (sb) {
            addFixed("Gate-Fixed-SchoolBreak-WakeUp-Vib");               // R2.1
          } else {
            addFixed("Gate-Fixed-Workday-WakeUp-Vib");                   // R2.2
            const yRest = (S("presence", addDays(d, -1)) || {}).rest;
            if (yRest) addFixed("Gate-Fixed-FirstWorkday-WakeUp-Ring");  // R2.3
          }
        }
        if (p.noon === "work")    fixed.push("Gate-Fixed-Workday-NapEnd-Vib");   // R2.4/R4.2
        if (p.evening === "work") fixed.push("Gate-Fixed-Workday-OffWork-Vib");  // R2.4/R4.3
      }
