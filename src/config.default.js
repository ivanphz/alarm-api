...
export const DEFAULT_CONFIG = {
  ...
  //
  // 来源: 工作事件晨间碰撞新建的唤醒闹钟(Gate-Dynamic-Event) + 周末上课(Gate-Class)。
  // （上课闹钟 V11.1 起也是动态 Gate-Class-*，见 WEEKEND_CLASS。）
  //
  ...
  // ───────────────────────────────────────────────────────────────────────────
  DYNAMIC_LABELS: {
    EVENT: "Gate-Dynamic-Event"   // 事件闹钟基名，实际标签追加 -HHMM
  },

  // 上课闹钟的两种标签（同一节课可能走其中之一，取决于当天时段时间是否==锚时段时间）
  //   FIXED   预建可开关: Gate-Fixed-Class-<id>       ← 需手机预建，常驻可靠，不进 sweep
  //   DYNAMIC 网关动态建: Gate-Class-<星期>-<id>-<HHMM> ← 无需预建，时间编入标签，进 sweep
  CLASS_LABELS: {
    FIXED:   "Gate-Fixed-Class",
    DYNAMIC: "Gate-Class"
  },
  ...
  //           · 你已在手机预建一条 Gate-Fixed-Class-<id>，时间 = periods[fixed]
  //           · 当天时段时间 == periods[fixed] → 走【固定】(开这条预建闹钟，常驻可靠)
  //           · 当天时段时间 != periods[fixed] → 走【动态】Gate-Class-<星期>-<id>-<HHMM>
  //           不写 fixed = 该课全部时段都走动态
  //
  // ⚠️ 固定那条的时间【真相在手机】: periods[fixed] 只是镜像 + 用于"该时段能否复用固定"判定。
  //    在手机改了 Gate-Fixed-Class-<id> 的时间，务必把 periods[fixed] 同步改，否则判定错位。
  //
  ...
  WEEKEND_CLASS: {
    ...
    SCHEDULE: [
      ...
      { day: 6, id: "sat-dance", name: "舞蹈课",
        ...
        fixed: "normal" }        // 手机预建: Gate-Fixed-Class-sat-dance @07:45
