...
export const CONFIG = {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 🔐 鉴权密钥【不在本文件里】—— 存 Worker Secret，变量名: GATEWAY_KEY
  //    所有请求 URL 必须带 ?key=<该密钥>，否则一律 401（fail-closed，绝不裸奔）。
  //
  //    配置（一次性，执行后粘贴你自定义的一串随机字符 + 回车）:
  ...
  //    浏览器调试时也要拼上 &key=你的密钥。
  //    密钥想换: 重新 put 一次覆盖，并把手机上 7 个 URL 同步改掉。
  // ───────────────────────────────────────────────────────────────────────────
