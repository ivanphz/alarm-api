...
1. **你负责"算成哪一刻"。** 工作日、间隔、自然月去重、跳节假日、动态改期…这些复杂计算
   ...
   重复闹钟即可，别走这个接口。
2. **每条闹钟带一个唯一且稳定的 uid。** 用来认"是不是同一条"。
   - **唯一**：不同闹钟不同 uid。建议 ASCII，可读（`cmb-young-0728`）或随机（crc32 都行）。
   - **稳定**：同一条闹钟每次拉取吐**同一个 uid**，直到它不该再响就别再吐。
     （uid 变了 = 我这边认成新闹钟，旧的关掉新的建，会抖。）
   - uid 代表"某一次具体响铃"还是"某件周期的事"由你定，但影响改期表现：
     · uid 含日期(`checkin-20260801`) → 改期 = 旧的消失、新的出现；
     · uid 不含日期(`checkin-202608`) → 改期 = 同一条挪时间，更平滑。
