# TODO-CHANNEL.md — todo 执行通道 + Bark 推送通道（实施规格，待建）
...
> V12/v2，勘误补丁不再单独存在**——本文即最终坐标，冲突时以本文为准，架构听 KERNEL.md。
>
> **配套必读**: KERNEL.md（宪法）· CHANNELS.md（通道能力实测台账，选型判据）·
> RULEBOOK.md（改法）· PHONE.md（执行器）。
...
| 要加什么 | 落点 |
|---|---|
| todos 节 | **`/v2/state` 信封新增 `todos` 节**（与 `alarms` 节并列；/v1 一个字节不动） |
| 对账提示标志 | **`reconcile_todos`**（与 `reconcile_alarms` 同构的 reconcile_* 命名族） |
...
| Bark 出站 | `edge/push.js`（新）；`BARK_KEY` 走 Cloudflare Secret |
...
> 本节是**行为契约**，据此在 PHONE.md 展开为逐动作脚本。
> 全部机制均有实测支撑（CHANNELS §6），**实施时不得凭直觉翻案**。
> PHONE.md 铁则四条适用：文本比较 / 标记判空 / 守卫不落账 / 命名八名封顶。
...
- 新 token 补进 KERNEL §18：`landing`、`reconcile_todos`、`gate-td://`、`gate-tdx://`。
- 改了信封 → **同步更新 PHONE.md 新增 §SyncTodos 章**。
