...
现有各时刻刺客可整体保留，仅把 URL 换成 `/v2/state?mode=point&now=HH:MM`（now 可省，
网关自取上海此刻），读 `fields.<f>.changes[]`——空数组 = 装死，有条目按 value 执行、
同样过 §3b/§3c 防线。**建议**: 先切主轮询（§3）灰度一周，刺客随后换或直接退役
（segment 每小时轮询 + 手动补跑，已覆盖刺客职能；保留 07:40 一个刺客做低延迟即可）。
