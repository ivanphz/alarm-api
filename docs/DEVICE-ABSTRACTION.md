...
> **状态**: ✅ **服务端已实施（2026-07-25）**，手机端待装配（见 `PHONE.md` v4.3）。
> **实施时相对本文做了三处修正，以实际代码与 `CONTRACT.md` 为准**：
> ① **App 表改两层**：原设计"类别表 + 具体App表"两张平表并列，同一个 bundle id 写两遍。
>    现为 `APPS`（原子层，bundle id 只写一次）+ `GROUPS`（类别层，只有 token、平台无关），
>    下发前拍平。加安卓只填原子层，类别自动复用。
> ② **resolve 表名 = 守卫 source 名**（`current_focus`/`app`/`locked`），并加 `locked` 恒等表，
>    让"无需翻译"的源也走同一句展开逻辑。
> ③ **守卫由服务端预展开** `match[]`：手机端 CheckGuards 因此不需要 resolve、少一层循环，
>    展开语义由服务端测试覆盖。
