# FEEDBACK-SELFHEAL.md — 手机状态回传与闹钟对账自愈（方向定稿，分步实施）
...
## 6. 与其他线的关系（防冲突）
- **与延迟实验(PHONE.md §11)同源**: applied_state 回传顺带就是延迟探针的被动数据;
...
- **与 todo 通道(P1)复用**: 自愈的提醒走的就是 P1 建好的 reconcile_todos 通道(TODO-CHANNEL.md)，
