# 10-SELFHEAL — 回传与自愈（设计定稿，待实施）

> **下一个大块。** 服务端批量回传端点已就绪（[03-CONTRACT](03-CONTRACT.md) §5），手机端未做。
...
> 与 05-FACTS.md 强绑定: 自愈的输出通道(reminder/Bark)选型服从其实测台账。
...
## 1.5 ⚠️ 三处与现状的差异（2026-07-31 核对）

本稿是 2026-07-17 定的方向，主体仍然成立。三处要按现状调整：

| 原稿 | 现状 |
|---|---|
| 「每字段动手成功后一条」POST | **改用批量端点**：`POST /v2/fact { events: [...] }`，一次连接发全部字段。锁屏后台预算只有 40–80 秒，三次往返太奢侈。形状见 [03-CONTRACT](03-CONTRACT.md) §5 |
| `silent` 不可观测，只能回传「我做了什么」 | **已可观测** —— Actions app 的 `Is Silent Mode On`（[05-FACTS](05-FACTS.md) §0.1）。三个字段现在全部读得回来，观测是完整的 |
| 「P-观测显示期望 vs 实测」 | 期望侧要注意：**全 pulse 之后段查询看不见 focus/silent**，并排比对时期望值要按**刺客时刻的 point 采样**取，不能用段采样 |

另：`run_id` 已由服务端在信封的 `telemetry` 节下发，回传时带上它，
就能把手机端本地日志（[04-PHONE](04-PHONE.md) §埋点）与服务端记录拼成一条链路。
...
**命名(遵 [09-KERNEL](09-KERNEL.md) §18)**: stream 名 snake_case; drift 分级 token critical/high/normal;
自愈补建的闹钟仍走 GateDyn-* 既有标签族(不新增前缀)。

---

## 6. 与其他线的关系（防冲突）
- **与延迟实验(04-PHONE.md §11)同源**: applied_state 回传顺带就是延迟探针的被动数据;
  colo/received_at 已在 /v2/fact 埋好。P-观测启用 = 延迟数据自动开始积累。
- **与 cadence(P4)正交**: cadence 产 todo/闹钟是"计划内生成"; 自愈是"计划外纠偏"。
  两者都汇入 todos 节/闹钟集，但来源不同、互不依赖。
- **与 todo 通道(P1)复用**: 自愈的提醒走的就是 P1 建好的 reconcile_todos 通道(11-TODO-CHANNEL.md)，
  所以**实施顺序: P1 todo 通道 → P2 Bark → P-观测回传 → P-自愈**，自愈是最后拼装，
  前面每块都为它备好了零件。

---

## 7. 实施顺序建议（已并入 HANDOFF §6 路线图）
```
