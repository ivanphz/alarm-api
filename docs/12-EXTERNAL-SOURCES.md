# 12-EXTERNAL-SOURCES — 外部闹钟源接入规范

> 给**提供源的一方**看的（自己的其它项目、脚本）。网关侧怎么配见 [06-OPERATIONS](06-OPERATIONS.md) §0.2。
...
### X.1 识别机制：强制识别，不做「全收」默许

| 格式 | 怎么认 |
|---|---|
| **JSON** | 每条必带 `uid`，有合法 uid 即准入 |
| **ICS** | 扫描每个 VEVENT 的**全字段拼接**，用 `markPattern`（默认 `[[ES:uid]]`）匹配。<br>命中 = 准入，捕获组 = uid；裸 `[[ES]]` 无捕获 → 回退原生 `UID`；未命中 = 不采纳 |

「标签放任意字段都行」就是靠扫全字段实现的 —— 标题/备注/CATEGORIES/X- 都在里面。

### X.2 ⚠️ 为什么时间必须编进 label（血泪教训，勿删）

外部源闹钟 label = `GateDyn-ES-<code>-<uid>-<HHMM>`。

手机端只能**按名称**比对，而 iOS 快捷指令**没有「改现有闹钟时间」的动作**。
所以「改时间生效」的**唯一**机制 = label 变 → 旧的对账关掉、新时间重建。

**若 label 不含时间**：同 uid 改时间 → 按名称找到旧的 → 只 Turn On →
**时间永不更新，静默失效。**

### X.3 排错日志怎么读

```
[外部闹钟] 🌐 <名>(<code>): 候选N 窗口内新增M
           (拒/警: 无标签a / 全天b / 无uid c / 格式d / 窗口外e / 时区未识别f)
```

| 哪项多 | 说明 |
|---|---|
| **无标签** | 源的 ICS 没放 `[[ES:uid]]` |
| **无 uid** | uid 缺失，或净化后为空（多为非 ASCII 主体） |
| **窗口外** | 吐了过去的点或太远的点（>24h）。提醒源只吐未来 |
| **时区未识别** | `tz` 写了个 `Intl` 不认的名，已按东八区兜底，去核对 |

### X.4 不合规的第三方源怎么接

自己的项目改字段即可。万一某个源改不动（真·第三方 ICS）：
**在网关和该源之间放一个转换 Worker**，把原始内容读进来、补上标签再吐给网关。
...
JSON 顶层可选 `v`；未知字段双向忽略；条目可选 `tz`；`allDay` 是枚举；
`GateDyn-ES-` 前缀固定而 `code`/`uid` 结构各自可演进；`markPattern` 可按源覆盖。